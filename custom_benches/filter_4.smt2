...
(rule (=> (and (inv a b n lbound ubound i bl) (>= i n)
  (not (forall ((pos Int)) (=>
      (and (<= 0 pos) (< pos n))
      (and
        (=> (and (>= (select a pos) lbound) (<= (select a pos) ubound))
          (exists ((pos2 Int)) (= (select b pos2) (select a pos)))
        )
        (=> (or (< (select a pos) lbound) (> (select a pos) ubound))
          (not (exists ((pos2 Int)) (= (select b pos2) (select a pos))))
        )
      )
  ))))  
	  fail))
