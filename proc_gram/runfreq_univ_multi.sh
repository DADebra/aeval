...
mkdir -p logs
exec > logs/log-frequnivmulti-$nowtime.txt 2>&1
ln -sf log-frequnivmulti-$nowtime.txt logs/log-frequnivmulti-latest.txt
...
dobench() {
    benchpath="$1"
    benchname="${benchpath#../bench_horn_multiple/}"
    echo "$benchname"
