...
namespace expr
{
  ...
  public:
    ...
    void push_back (ENode* a) { args.push_back (a); a->Ref (); }
  ...
  struct ENodeUniqueHash
  {
	...
	boost::hash_combine (res,
			     boost::hash_range (it, e->args_end ()));
  ...
  struct ENodeUniqueEqual
  {
	...
	if (e1->arity () == e2->arity ())
	  ...
	  if (e1->op () == e2->op ())
	    ...
	    return std::equal (e1->args_begin (),
			       e1->args_end (),
			       e2->args_begin ());
      return false;
    }
  ...
  struct LessENode
  {
	...
	{
	  if (e1->op () == e2->op ())
	    return lexicographical_compare (e1->args_begin (),
					    e1->args_end (),
					    e2->args_begin (),
					    e2->args_end ());
	  else
  ...
  template <typename C>
  struct CacheStubTmpl : CacheStub
  {
    C &cache;

    CacheStubTmpl (C &c): cache(c) {};

    virtual bool owns (const void *p) { return p == static_cast<const void*> (&cache); }
...
#define UNORDERED_SET_UNIQUE_TABLE 1
      ...
      if (!val->isMutable ())
	{
	  ...
	  it->second.erase (val);
	  if (it->second.empty ()) unique.erase (it);
	}
    ...
    ENode* mkExpr (const Operator &op)
    { return canonize (allocNode (op)); }

    template <typename etype>
    ENode* mkExpr (const Operator &op, etype e)
    {
    ...
    */
    template <typename iterator>
    ENode* mkNExpr (const Operator &op,
		    iterator begin,
		    iterator end)
    {
      ENode* eVal = allocNode (op);
...
#define FREE_LIST_MAX_SIZE 1024*4
  ...
  public:
    ...
    {
      val->Deref ();
      if (val->isGarbage ()) Remove (val);
    }

    /** User functions */
    Expr mkTerm (const Operator &o) { return Expr (mkExpr (o)); }
    Expr mkUnary (const Operator &o, Expr e)
    { return Expr (mkExpr (o, e.get ())); }
    Expr mkBin (const Operator &o, Expr e1, Expr e2)
    { return Expr (mkExpr (o, e1.get (), e2.get ())); }
    Expr mkTern (const Operator &o, Expr e1, Expr e2,
		 Expr e3)
    ...
    template <typename Cache>
    bool unregisterCache (const Cache &cache)
    {
  ...
  inline ENode::ENode (ExprFactory &f, const Operator &o) :
    count(0), fac(&f),
    oper(o.clone (f.allocator),
	 f.allocator.get_deleter (),
...
namespace expr
{
  ...
  inline ENode *ExprFactory::allocNode (const Operator &op)
  {
    ...
    freeList.pop_back ();
    res->oper.reset (op.clone (allocator),
  ...
  {
    if (n <= tiny.get_requested_size ()) return tiny.malloc ();
    else if (n <= small.get_requested_size ()) return small.malloc ();
  ...
  {
    if (tiny.is_from (block)) tiny.free (block);
    else if (small.is_from (block)) small.free (block);
    else delete [] static_cast<char * const> (block);
  }

  inline EFADeleter ExprFactoryAllocator::get_deleter ()
  { return EFADeleter (*this); }

  inline void EFADeleter::operator() (void *p)
  { operator delete (p, m_efa); }

  template <typename T>
  struct TerminalTrait {};
  ...
  template<> struct TerminalTrait<int>
  {
    ...
    { OS << s; }
    static inline bool less (const int &i1, const int &i2)
    { return i1 < i2; }
    static inline bool equal_to (int i1, int i2)
    { return i1 == i2; }
  ...
  {
    static inline void print (std::ostream &OS,
			      unsigned long s, int depth, bool brkt)
    { OS <<  s; }
...
//  space (OS, depth);
  ...
  inline ENode::~ENode ()
  {
    for (args_iterator b = args.begin (), e = args.end ();
	 b != e; ++b)
      efac().Deref (*b);
  }
  ...
  template <typename iterator>
  void ENode::renew_args (iterator b, iterator e)
  {
    ...
    for (args_iterator b = old.begin (), e = old.end ();
	 b != e; ++b)
      efac().Deref (*b);
  }
  ...
  inline void intrusive_ptr_add_ref (ENode *v)
  {
    v->Ref ();
  }
  ...
  Expr visit (ExprVisitor &v, Expr expr)
  {
    ...
    if (va.isSkipKids ()) return expr;

    if (va.isChangeTo ()) return va.getExpr ();

    Expr res = va.isChangeDoKidsRewrite () ? va.getExpr () : expr;

    if (res->arity () == 0) return va.rewrite (res);
   ...
   * Usage: mk<TRUE> (efac)
   */
  template <typename T> Expr mk (ExprFactory &f)
  { return f.mkTerm (T()); }
   ...
   * Usage: mk<NEG> (exp)
   */
  template <typename T> Expr mk (Expr e)
  { return e->efac ().mkUnary (T(), e); }

  template <typename T> Expr mk (Expr e1, Expr e2)
  { return e1->efac ().mkBin (T(), e1, e2); }

  template <typename T> Expr mk (Expr e1,
				 Expr e2,
				 Expr e3)
  ...
  {
    if (bgn == end) return base;
    if (std::distance (bgn, end) == 1) return eptr (*bgn);
  ...
  Expr mknary (const Range &r)
  { return mknary<T> (boost::begin(r), boost::end(r)); }

  template<typename T, typename Range>
  Expr mknary (Expr base, const Range &r)
   ...
   * Usage: mk (op, efac)
   */
  inline Expr mk (const Operator &op, ExprFactory &f)
  { return f.mkTerm (op); }

  inline Expr mk (const Operator &o, Expr e)
  { return e->efac ().mkUnary (o, e); }
  ...
  namespace op
  {
	...
	if (isOpX<TRUE>(e1)) return e2;
	if (isOpX<TRUE>(e2)) return e1;
	...
	return mk<AND>(e1, e2);
      ...
      {
        if (isOpX<FALSE>(e1)) return e2;
        if (isOpX<FALSE>(e2)) return e1;
        if (isOpX<TRUE>(e1) || isOpX<TRUE>(e2)) return mk<TRUE>(e1->efac ());
        return mk<OR>(e1, e2);
      }
	...
	// TRUE -> x  IS x
        if (isOpX<TRUE> (e1)) return e2;
	// x -> TRUE  IS TRUE
	if (isOpX<TRUE> (e2)) return e2;
	// FALSE -> x IS TRUE
	if (isOpX<FALSE> (e1)) return mk<TRUE> (e1->efac ());
	// x -> x IS TRUE
	if (e1 == e2) return mk<TRUE> (e1->efac ());
	...
	// x -> FALSE is missing since it adds a negation
      ...
      {
	if (isOpX<TRUE>(e1)) return mk<FALSE> (e1->efac ());
	if (isOpX<FALSE>(e1)) return mk<TRUE> (e1->efac ());

        if (isOpX<NEG>(e1)) return e1->left();
	...
	if (boost::size (r) == 1) return *boost::begin (r);
	...
	{
	  if (exp == trueE || exp == falseE) return exp;

	  if (!isOp<BoolOp> (exp)) return exp;
	  ...
	  if (isOpX<IMPL>(exp))
	    {
          ...
          if (isOpX<IFF> (exp))
            {
              if (exp->left () == exp->right ()) return exp->left ();
              if (trueE == exp->left ()) return exp->right ();
              if (falseE == exp->left ()) return lneg (exp->right ());
              if (trueE == exp->right ()) return exp->left ();
              if (falseE == exp->right ()) return lneg (exp->left ());
	  ...
	  if (isOpX<OR> (exp))
	    {
	      if (arity == 0) return falseE;
	      if (arity == 1) return exp->left ();
	      if (arity == 2)
		{
		  ENode* lhs = exp->left ();
		  ENode* rhs = exp->right ();
	...
	Expr operator() (Expr exp)
	{
	  ...
	  if (! (isOpX<AND> (exp) || isOpX<OR> (exp)) ) return exp;
	  ...
	  if (newArgs.empty ()) return top;
	  if (newArgs.size () == 1) return *(newArgs.begin ());
	...
	NormalizeOps (const NormalizeOps &o) :
	  trueE (o.trueE), falseE (o.falseE) {}
	...
	Expr operator() (Expr exp)
	{
	  ...
	  if (exp->arity () == 0) return top;

	  if (exp->arity () == 2)
	    {
	      if (isOpX<AND> (exp)) return land (exp->left (), exp->right ());
	      else /* isOpX<OR> */ return lor (exp->left (), exp->right ());
	    }
	...
	Expr land (Expr f, Expr g)
	{
	  ...
	  if (f == trueE) return g;
	  if (g == trueE) return f;
	  if (f == falseE || g == falseE) return falseE;
	  if (f == g) return f;
	  if (f == boolop::lneg (g) || boolop::lneg (f) == g) return falseE;
	...
	{
	  /** base cases */
	  if (f == falseE) return g;
	  if (g == falseE) return f;
	  if (f == trueE || g == trueE) return trueE;
	  if (f == g) return f;
	  if (f == boolop::lneg (g) || boolop::lneg (f) == g) return trueE;
	...
	VisitAction operator() (Expr exp)
	{
	  ...
	  if (!isOpX<NEG> (exp)) return VisitAction::skipKids ();

	  // -- if here, top operator is negation, push it in
	  Expr lhs = exp->left ();
	  if (lhs == r->falseE) return VisitAction::changeTo (r->trueE);
	  if (lhs == r->trueE) return VisitAction::changeTo (r->falseE);
	  ...
	  if (isOpX<OR> (lhs) || isOpX<AND> (lhs))
		...
		args.push_back (lneg (arg));
	      ...
	      // -- flip operator
	      Expr res = isOpX<OR> (lhs) ?
		mknary<AND> (args.begin (), args.end ()) :
  ...
  namespace op
  {
	...
	if (isOpX<FALSE>(e1)) return mk<TRUE> (e1->efac ());
        ...
        if (isOpX<NEG_G>(e1) || isOpX<NEG> (e1)) return e1->left();
 ...
 namespace op
  ...
  namespace op
  {
    namespace array
    {
      ...
      inline Expr constArray (Expr domain, Expr v) {return mk<CONST_ARRAY> (domain, v);}
  ...
  namespace op
  {
    ...
    namespace bind
    {
      inline Expr bind (Expr name, Expr value)
      { return mk<BIND> (name, value); }
...
namespace expr
{
  ...
  template<> struct TerminalTrait<op::bind::BoundVar>
  {
    ...
    static inline size_t hash (const op::bind::BoundVar &b)
    { return b.hash (); }
  };
  ...
  namespace
  {
	...
	s(_s), t(_t), r(std::make_shared<boolop::TrivialSimplifier> (s->efac ())) {}
      ...
      VisitAction operator() (Expr exp) const
      {
	if (exp == s) return VisitAction::changeTo (t);
	return VisitAction::changeDoKidsRewrite (exp, r);
      }
    };
    ...
    struct FV : public std::unary_function<Expr,VisitAction>
    {
      ...
      VisitAction operator() (Expr exp)
      {
	if (seen.count (exp) > 0) return VisitAction::skipKids ();
	seen.insert (exp);
  ...
  struct fn_map
  {
    ...
    {
      Expr res = f(exp);
      if (res) return const_iterator (exp, res);
      return end ();
    }
  ...
  template <typename F>
  fn_map<F> mk_fn_map (const F &fn) { return fn_map<F> (fn); }

  template <typename M>
  Expr replace (Expr exp, const M &map)
  {
    RV<M> rv(map);
  ...
  template <typename M>
  Expr replaceSimplify (Expr exp, const M &map)
  {
    ...
    namespace bind
    {
      ...
      inline Expr typeOf (Expr v)
      {
        ...
        if (isOpX<STORE>(v))
        {
          ...
          return sort::arrayTy(typeOf(v->right()), typeOf(v->last()));
        }
...
namespace expr
{
  ...
  class ExprCache
  {
    typedef boost::bimaps::bimap<boost::bimaps::unordered_set_of<ENode*>,
				 boost::bimaps::list_of<T> > cache_type;
  ...
  public:
    ...
    void clear ()
    {
      for (iterator it = cache.left.begin (), end = cache.left.end ();
	   it != end; ++it)
	{
	  ENode *n = it->first;
	  n->efac ().Deref (n);
