...
(define-fun solution_inv (
  ...
  (N Int)) Bool
    (and
      ...
      (forall ((j Int))                     ; lemma from count_occs_0
         (=> (forall ((k Int))
           (=> (and (<= 0 k) (< k i)) (not (= j (select A k)))))
             (= (select P j) -1)))

      (forall ((j Int))                     ; lemma from count_occs_1
        (=> (and (<= 0 j) (< j len))
          (exists ((k Int))
            (and (<= 0 k) (< k i)
              (= (select B j) (select A k))))))

     (forall ((j Int))                     ; lemma from count_occs_2
        (=> (and (<= 0 j) (< j i))
            (exists ((k Int))
              (and (<= 0 k) (< k len)
                (= (select A j) (select B k))))))

      (forall ((j Int))                    ; lemma from count_occs_3
        (=> (and (<= 0 j) (< j i)) (>= (select P (select A j)) 0)))


     (forall ((j Int) (k Int))              ; property itself (made inductive)
        (=> (and (<= 0 j) (< j k) (< k len))
              (not (= (select B j) (select B k)))))
    ))
...
(rule (=> (and (inv A B P C i len N) (>= i N)
  (not (forall ((j Int) (k Int))
     (=> (and (<= 0 j) (< j k) (< k len))
           (not (= (select B j) (select B k))))))) fail))
